...
namespace SharpGame
{
    ...
    public class Model : Resource
    {
        ...
        static VertexLayout CreateVertexInputStateCreateInfo(uint mask, out uint stride)
        {
            ...
            {
                vertexInputAttributes.Add(new VertexInputAttribute(location, 0, Format.R32g32b32Sfloat, stride));
                stride += 12;
                location++;
            }
            ...
            {
                vertexInputAttributes.Add(new VertexInputAttribute(location, 0, Format.R8g8b8a8Unorm, stride));
                stride += 4;
                location++;
            }
            if ((mask & MASK_TEXCOORD1) != 0)
            {
                vertexInputAttributes.Add(new VertexInputAttribute(location, 0, Format.R32g32Sfloat, stride));
                stride += 8;
                location++;
            }
            ...
            {
                vertexInputAttributes.Add(new VertexInputAttribute(location, 0, Format.R32g32b32a32Sfloat, stride));
                stride += 16;
                location++;
            }
            ...
            {
                vertexInputAttributes.Add(new VertexInputAttribute(location, 0, Format.R8g8b8a8Uint, stride));
                stride += 4;
                location++;
            }
        ...
        protected override void OnBuild()
        {
            ...
            for (int i = 0; i < vertexBuffers_.Length; ++i)
            {
                ...
                if (desc.data_ != null)
                {
                    buffer = GraphicsBuffer.Create(VkBufferUsageFlags.VertexBuffer, false
            ...
            for (int i = 0; i < indexBuffers_.Length; ++i)
            {
                ...
                if (desc.data_ != null)
                {
                    buffer = GraphicsBuffer.Create(VkBufferUsageFlags.IndexBuffer, false, desc.indexSize_, desc.indexCount_, Utilities.AsPointer(ref desc.data_[0]));
                }
            }
